...
if [[ $sayi -gt 0 ]]; then
   ...
   if (( cat_kontrol == 1 )); then
      ...
      index=$((1 + $RANDOM % 1100))
      rclone move /data2/plor3/ cript$account:max --drive-service-account-file=/root/.config/rclone/henry/"$index.json" --drive-server-side-across-configs --user-agent="${KT}" --drive-chunk-size=2G --drive-upload-cutoff=1000T --drive-stop-on-upload-limit --retries=1 --checkers=3 --tpslimit=3 --transfers=6 -P
      account=$((1 + $RANDOM % 30))
   else
