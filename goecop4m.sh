...
account=$((1 + $RANDOM % 30))
cat_kontrol=0
echo -e "daenwi" >> "deneme.txt"
while :
do
sayi=0
sayi=($(ls /data2/plor4/*.plot | wc -l))
KT="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36"
...
if [[ $sayi -gt 0 ]]; then
   ...
   if (( cat_kontrol == 1 )); then
      ...
      index=$((1 + $RANDOM % 1100))
      rclone move /data2/plor4/ cript$account:max --drive-service-account-file=/root/.config/rclone/henry/"$index.json" --drive-server-side-across-configs --user-agent="${KT}" --drive-chunk-size=2G --drive-upload-cutoff=1000T --drive-stop-on-upload-limit --retries=1 --checkers=3 --tpslimit=3 --transfers=6 -P
      account=$((1 + $RANDOM % 30))
   else
